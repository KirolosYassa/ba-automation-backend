...
from UML_classdiagramNew.helperFunctions import nlp
import regex
...
def preprocess(text):
    ...
    for token in doc:
        ...
        if token.dep_ == "compound":
            text = text.replace(
                token.text + " " + token.head.text, token.text + "_" + token.head.text
            )
        ...
        if (
            ...
            and token.head.tag_ == "VBG"
        ):
            text = text.replace(
                token.head.text + " " + token.text, token.head.text + "_" + token.text
            )
...
def calculate_word_frequencies(sentences):
    ...
    word_frequencies = {
        key: wordsCountInAllSent[key] / len(sentences.keys())
        for key in wordsCountInAllSent.keys()
    }
...
def ExtractAttributes(sentences):
    ...
    attp2 = [
        ...
        {"POS": "NOUN"},
        {"POS": "AUX", "LOWER": {"IN": AUX}},
        {"POS": "VERB", "DEP": "ROOT", "LOWER": {"IN": attp2Verbs}},
        {"POS": "ADP"},
        {"OP": "+"},
    ]
    ...
    for sentence in sentences.sents:
        ...
        for match_id, start, end in matches:
            string_id = helperFunctions.nlp.vocab.strings[
                match_id
...
def ExtractInheritanceR(sentences):
    ...
    TYPEWORD = ["types", "type"]
    inherRp1 = [
        {"POS": "NOUN", "DEP": "nsubj", "LOWER": {"IN": TYPEWORD}},
        {"POS": "ADP", "DEP": "prep"},
        {"POS": "NOUN", "DEP": "pobj"},
        ...
        for match_id, start, end in matches:
            ...
            for I, token in enumerate(span):
                ...
                if string_id == "inheritanceRelationPattern1":
                    ...
                    if token.pos_ == "NOUN" and (
                        token.dep_ == "attr" or token.dep_ == "conj"
                    ):
                ...
                if string_id == "inheritanceRelationPattern4":
                    if token.pos_ == "NOUN" and token.dep_ == "nsubj":
                        ...
                        token.dep_ == "pobj" or token.dep_ == "conj"
                    ):
                        classIs = token.text.lower()
        ...
        for match_id, start, end in matches:
            ...
            for I, token in enumerate(span):
                ...
                if string_id == "aggregationRelationPattern1":
                    ...
                    if token.pos_ == "NOUN" and (
                        token.dep_ == "dobj" or token.dep_ == "conj"
                    ):
                        rels.append(token.lemma_.lower())
            ...
            for I, token in enumerate(span):
                ...
                if string_id == "compositionRelationPattern2":
                    ...
                    if token.pos_ == "NOUN" and (
                        ...
                        token.dep_ == "appos" or token.dep_ == "conj"
                    ):
                        rels.append(token.lemma_.lower())
...
def ExtractMethods(sentences):
    ...
    for sentence in sentences.sents:
        ...
        if foundClassName.text.lower() in classes:
            ...
            sentence[0].pos_ == "PROPN"
            or sentence[0].pos_ == "NOUN"
            or sentence[0].pos_ == "INTJ"
        ) and sentence[0].dep_ == "nsubj":
        ...
        if len(object):
            ...
            object_dependents_Txt = [
                token.text for token in object[0].subtree if token.i > object[0].i
            ]
