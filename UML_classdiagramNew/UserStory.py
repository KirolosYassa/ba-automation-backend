...
from UML_classdiagramNew.UseCase.Actor import Actor
...
class UserStory:
    ...
    def extractUseCase(sentence):
        ...
        foundActor = [
            ...
            if actorName.name == actorExtracted.name
        ]
    ...
    def extractDependencyUseCase(sentence, verb, object_dependents):
        ...
        if len(object_dependents):
            ...
            verb_dependents_Txt = [
                token.text for token in verb.subtree if token.i > verb.i + 1
            ]
            ...
            verb_dependents_Txt = [
                token.text for token in verb.subtree if token.i > verb.i
            ]
            print("verb_dependents", verb_dependents_Txt)
            ...
            if len(verb_dependents):
                for i, tok in enumerate(verb_dependents):
                    if (
                        verb_dependents[i].dep_ == "prep"
                        and verb_dependents[i].pos_ == "ADP"
                        and verb_dependents[i].lemma_ in WORDS
                    ):
                        print(verb_dependents[i].text)
                        dependents = [
                            token
                            for token in verb.subtree
                            if token.i > verb_dependents[i].i
                        ]
                        dependents_txt = [
                            token.text
                            for token in verb.subtree
                ...
                for k, tok in enumerate(doc):
                    if tok.i >= token.i:
                        ...
                        if (
                            tok.dep_ == "prep"
                            and tok.pos_ == "ADP"
                            and tok.text not in WORDSDependency
